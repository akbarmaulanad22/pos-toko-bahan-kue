        ...
        <form class="row g-3" method="POST" action="{{ route('stockflow.update', ['stockflow' => $stockflow]) }}"
            ...
            <div class="col-md-3">
                ...
                <select class="form-select" id="type" name="type" aria-describedby="typeFeedback" required>
                    ...
                    <option value="MISSING" {{ old('type', $stockflow->type) == 'MISSING' ? 'selected' : '' }}>
                        Missing
                    </option>
                </select>
                @error('type')
...
@push('scripts')
    <script>
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    data.forEach(size => {
                        ...
                        if (size.id == {{ old('size_id', $stockflow->size_id) }}) {
        ...
        handleChangeProduct(document.getElementById('product_id'))
    </script>
@endpush
